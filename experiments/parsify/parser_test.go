	...
	for _, tc := range specs {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.basicParser2 != nil {
				state, firstOutput = tc.basicParser2(gomme.NewFromString(-1, nil, input[:1]))
			} else {
				state, firstOutput = tc.basicParser1.It(gomme.NewFromString(-1, nil, input[:1]))
			}
			...
			if tc.basicParser2 != nil {
				newState, gotOutput = tc.complexParser2(gomme.NewFromString(-1, nil, input))
			} else {
				newState, gotOutput = tc.complexParser1.It(gomme.NewFromString(-1, nil, input))
			}
